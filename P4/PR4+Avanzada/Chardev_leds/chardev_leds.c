...
device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
    ...
    printk(KERN_INFO "Updating leds \n");

    int bytes_to_write = len;
    char buffAux[BUF_LEN];

    if(copy_from_user(buffAux,buff,bytes_to_write)) {
        printk(KERN_INFO "Fail while executing copy_from_user \n");        
        return -EFAULT;
    }
    
    unsigned int mask = ALL_LEDS_OFF, i ;
