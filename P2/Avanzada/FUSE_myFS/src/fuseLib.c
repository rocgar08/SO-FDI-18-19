...
void debug(){
    printf("\n\n\n\n");
    printf("-----------------Cheking FileSystemData---------------\n");
    printf("MyFileSystemStructure Data: \n");
    ...
    printf("----------------->>> maxBlocksPerFile %i : \n",myFileSystem.superBlock.maxBlocksPerFile);

    printf("---------->>> bitMap DATA, %ld: \n",NUM_BITS);

    for (int i = 1; i < NUM_BITS; ++i){
        printf("%d ", myFileSystem.bitMap[i-1]);
        if(i%31==0)printf("\n");
    }
    ...
    for (int i = 0; i < myFileSystem.directory.numFiles; ++i){
        if(!(myFileSystem.directory.files[i].freeFile)) {
            printf("File %d \n", i);
            printf("------------------>>> numFiles %d: \n",myFileSystem.directory.files[i].nodeIdx);
            printf("------------------>>> fileName %s: \n",myFileSystem.directory.files[i].fileName);
            printf("------------------>>> freeeFile %d: \n",myFileSystem.directory.files[i].freeFile);
        }
    }
    ...
    for (int i = 0; i < MAX_NODES-myFileSystem.numFreeNodes; ++i){
        printf("---------->>> Nodes %i: \n",i);
        printf("------------------>>> numBlocks %d: \n",myFileSystem.nodes[i]->numBlocks);
        printf("------------------>>> fileSize %d: \n",myFileSystem.nodes[i]->fileSize);
        printf("------------------>>> links %d: \n",myFileSystem.nodes[i]->links);
        printf("------------------>>> modificationTime %ld: \n",myFileSystem.nodes[i]->modificationTime);
        printf("------------------>>> freeNode %d: \n",myFileSystem.nodes[i]->freeNode);    
    }
}
...
/**Implenta la operación para leer los datos de los ficheros asociados al miembro read de la estructura fuse_operations.
  *Dese cuenta de que , según el manual,la funcióm debe retornar tantos bytes como se solicitan siempre y cuando los haya, ya que en caso contrario serán rellenados con cers
  *(no se espedifica en el montaje de la unidad)
  ***/
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){

    char buffer[BLOCK_SIZE_BYTES];
    int bytes2Read = size, totalReaded = 0;
    NodeStruct *node = myFileSystem.nodes[fi->fh];

    fprintf(stderr,"->>->>my_read: path %s , link state %i \n",path,node->links);
    ...
    while(bytes2Read) {
        ...
        for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalReaded < size); i++) {
            buf[totalReaded++]=buffer[i];
        }
...
static int my_unlink(const char* path){
    ...
    fprintf(stderr,"->>>>my_unlink: archivo a eliminar %s \n",myFileSystem.directory.files[idxDir].fileName);
    myFileSystem.directory.numFiles--;
    
    if( myFileSystem.nodes[idxNode]->links == 1 ){
...
static int my_link(const char* from, const char* to){
    fprintf(stderr,"->>>>my_link: from %s , -- to %s \n",from+1,to+1);

    if(myFileSystem.directory.numFiles == MAX_FILES_PER_DIRECTORY){
        fprintf(stderr,"->>>>my_link: The directory space is full. \n");        
        return -EPERM;   
    }

    if(strcmp(from,to)==0){
        fprintf(stderr,"->>>>my_link: Both files have the same name, the operation is not allowed. \n");        
        return -EPERM;
    }

    int idxDirFrom, idxNodeShared;     //position of the file called like from, and position of the node shared by from and to files.

    if( (idxDirFrom = findFileByName(&myFileSystem,(char*)from+1)) == -1){
        fprintf(stderr,"->>>>my_link: The *from-file* dosen't exist \n");        
        return -ENOENT;
    }

    //inode where the data is saved.
    idxNodeShared=myFileSystem.directory.files[idxDirFrom].nodeIdx;
    
    //update of the directory.files data.
    myFileSystem.directory.files[myFileSystem.directory.numFiles].nodeIdx=idxNodeShared;
    strcpy(myFileSystem.directory.files[myFileSystem.directory.numFiles].fileName, to+1);
    myFileSystem.directory.files[myFileSystem.directory.numFiles].freeFile=false;
    myFileSystem.directory.numFiles++;

    //update of the numbers of links of the node.
    myFileSystem.nodes[idxNodeShared]->links++;
    
    //update the changes on disk.
    updateDirectory(&myFileSystem);
    updateNode(&myFileSystem, idxNodeShared, myFileSystem.nodes[idxNodeShared]);
...
struct fuse_operations myFS_operations = {
	...
	.unlink     = my_unlink,                    // unlink file
	.read       = my_read,                      // readFile
    .link       = my_link,                       // to make hard links           
