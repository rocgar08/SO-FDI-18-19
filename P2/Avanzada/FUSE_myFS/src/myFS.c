...
{   NodeStruct *temp=(NodeStruct*) malloc(sizeof(NodeStruct));

    for (int i = 0; i < MAX_NODES; i++){

        if( readNode(myFileSystem,i, temp) < 0){
            perror("Failed reading in readInodes");
            return -1;   
        }
        
        if(temp->freeNode==true) 
            myFileSystem->nodes[i]=NULL;
        else{
            myFileSystem->numFreeNodes--;
            myFileSystem->nodes[i]=malloc(sizeof(NodeStruct));
            copyNode(myFileSystem->nodes[i], temp);
        }
